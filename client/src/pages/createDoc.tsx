...
// import { Button, Input, Text, VStack } from 'rsuite';
...
// type Errors = {
//   clientInfo?: { fullName?: string; nationalId?: string; dateOfBirth?: string,id:number }[];
//   plaintiff?: string;
//   defendant?: string;
//   proceedingNumber?: string;
//   phoneNumber?: string;
...
//     { id: 1, fullName: '', dateOfBirth: '', nationalId: '' },
//   ]);
...
//   const [isAddClientButtonDisabled, setIsAddClientButtonDisabled] = useState(true);

//   // Enable "Add Client" only if last client is complete
//   useEffect(() => {
//     const last = clients[clients.length - 1];
//     const isComplete = last.fullName && last.nationalId && last.dateOfBirth;
//     setIsAddClientButtonDisabled(!isComplete);
//   }, [clients]);
...
//     setDefendants((prev) => prev.filter((item) => item !== tag));
//   };

//   // Remove plaintiff
//   const removePlaintiffTag = (tag: string) => {
//     setPlaintiffs((prev) => prev.filter((item) => item !== tag));
//   };

//   // Handle client info change
//   const handleClientInfoChange = (id: number, field: keyof ClientInfo, value: string) => {
//     // Find index before updating state
...
//     // Clear error if it exists for this field
//     if (errors.clientInfo?.[index]?.[field]) {
//       const newErrors = { ...errors };
//       newErrors.clientInfo = [...newErrors.clientInfo]; // make copy
//       newErrors.clientInfo[index] = {
//         ...newErrors.clientInfo[index],
//         [field]: undefined,
//       };
//       setErrors((prevErrors) => {
//         const index = clients.findIndex((c) => c.id === id);
//         if (index === -1) return prevErrors;
      
//         // Check if this specific error exists
//         if (!prevErrors.clientInfo?.[index]?.[field]) {
//           return prevErrors; // No error to clear
//         }
//               const newErrors = { ...prevErrors };
      
//         const clientErrors = [...(newErrors.clientInfo || [])];
      
//         // Extend the array if too short (e.g., new client added)
//         while (clientErrors.length <= index) {
//           clientErrors.push({fullName:"",nationalId:"",dateOfBirth:"",id:1,});
//         }
...
//     if (!lastClient.fullName || !lastClient.nationalId || !lastClient.dateOfBirth) {
//       // Trigger validation
//       const newErrors = { ...errors };
//       newErrors.clientInfo = clients.map((client, idx) => {
//         if (idx === clients.length - 1) {
//           return {
//             fullName: client.fullName ? undefined : 'نام و نام خانوادگی الزامی است',
//             nationalId: client.nationalId ? undefined : 'کد ملی الزامی است',
//             dateOfBirth: client.dateOfBirth ? undefined : 'تاریخ تولد الزامی است',
//           };
//         }
//         return errors.clientInfo?.[idx] || {};
//       });
//       setErrors(newErrors);
//       return;
//     }
...
//     setClients((prev) => [
//       ...prev,
//       { id: newId, fullName: '', nationalId: '', dateOfBirth: '' },
//     ]);
//   };
...
//     // Validate clients
//     newErrors.clientInfo = clients.map((client) => {
...
//     if (newErrors.clientInfo.every((err) => Object.keys(err).length === 0)) {
//       delete newErrors.clientInfo;
//     }
...
//     if (!formData.courtNumber) newErrors.courtNumber = 'شماره دادگاه الزامی است';
...
//               client={client}
//               errors={errors}
//               onClientInfoChange={(field, value) => handleClientInfoChange(client.id, field, value)}
//             />
//           ))}
...
//                     errors.plaintiff ? 'border-red-500' : 'border-gray-300'
//                   } rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500`}
//                   placeholder="نام خواهان"
//                 />
//                 <Button onClick={handleAddPlaintiff}>اضافه</Button>
//               </div>
//               {errors.plaintiff && <p className="mt-1 text-sm text-red-500">{errors.plaintiff}</p>}
...
//                   className={`flex-1 px-4 py-3 border ${
//                     errors.defendant ? 'border-red-500' : 'border-gray-300'
//                   } rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500`}
//                   placeholder="نام خوانده"
//                 />
//                 <Button onClick={handleAddDefendant}>اضافه</Button>
//               </div>
//               {errors.defendant && <p className="mt-1 text-sm text-red-500">{errors.defendant}</p>}
...
//                 errors.proceedingNumber ? 'border-red-500' : 'border-gray-300'
//               } rounded-lg`}
//               placeholder="مثلاً: 123-456-789"
//             />
//             {errors.proceedingNumber && (
//               <p className="mt-1 text-sm text-red-500">{errors.proceedingNumber}</p>
//             )}
//           </div>
...
//           {/* Court Number */}
//           <div>
//             <label className="block text-sm font-medium text-gray-700 mb-1">شماره دادگاه</label>
//             <Input
//               type="text"
//               name="courtNumber"
...
//               onChange={(value, e) => handleChange(value, e)}
//               className={`w-full px-4 py-3 border ${
//                 errors.courtNumber ? 'border-red-500' : 'border-gray-300'
//               } rounded-lg`}
//               placeholder="مثلاً: دادگاه کیفری مرکز 3"
//             />
//             {errors.courtNumber && (
//               <p className="mt-1 text-sm text-red-500">{errors.courtNumber}</p>
//             )}
//           </div>
...
//               appearance="primary"
//               type="submit"
//               className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-3 px-6 rounded-lg shadow transition transform hover:scale-[1.02]"
//             >
//               ثبت اطلاعات
//             </Button>
//           </div>
...
// }) {
//   const index = client.id - 1; // only for error indexing

//   return (
//     <div className="space-y-4 border-b border-blue-500 pb-6 mb-4">
//       <div>
...
//           onChange={(value) => onClientInfoChange('fullName', value)}
//           className={`w-full px-4 py-3 border ${
//             errors.clientInfo?.[index]?.fullName ? 'border-red-500' : 'border-gray-300'
//           } rounded-lg`}
//           placeholder="مثال: علی محمدی"
//         />
//         {errors.clientInfo?.[index]?.fullName && (
//           <p className="mt-1 text-sm text-red-500">{errors.clientInfo[index].fullName}</p>
//         )}
//       </div>

//       <div>
//         <label className="block text-sm font-medium text-gray-700 mb-1">کد ملی موکل</label>
//         <Input
//           value={client.nationalId}
//           onChange={(value) => onClientInfoChange('nationalId', value)}
//           className={`w-full px-4 py-3 border ${
//             errors.clientInfo?.[index]?.nationalId ? 'border-red-500' : 'border-gray-300'
//           } rounded-lg`}
//           placeholder="1234567890"
//           maxLength={10}
//         />
//         {errors.clientInfo?.[index]?.nationalId && (
//           <p className="mt-1 text-sm text-red-500">{errors.clientInfo[index].nationalId}</p>
//         )}
//       </div>
...
//           onChange={(value) => onClientInfoChange('dateOfBirth', value)}
//           className={`w-full px-4 py-3 border ${
//             errors.clientInfo?.[index]?.dateOfBirth ? 'border-red-500' : 'border-gray-300'
//           } rounded-lg`}
//         />
//         {errors.clientInfo?.[index]?.dateOfBirth && (
//           <p className="mt-1 text-sm text-red-500">{errors.clientInfo[index].dateOfBirth}</p>
//         )}
//       </div>
//     </div>
//   );
    ...
    <>
      ...
      <div>
        ...
        <Input
          ...
          className={`w-full ${error.dateOfBirth ? 'border-red-500' : 'border-gray-300'}`}
        />
